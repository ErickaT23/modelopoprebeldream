...
function resolveEventId() {
    ...
    const defaultEventId = String(eventConfig.defaultEventId || 'unforgettaval-valentina-2026').trim() || 'unforgettaval-valentina-2026';
...
function applySiteConfig() {
    ...
    const invitadoMensaje = document.querySelector('.invitado-mensaje, #rsvp-section .rsvp-intro');
    if (invitadoMensaje) invitadoMensaje.textContent = SiteConfig.textos.mensajeInvitado;

    applyEventCard('#ceremonia > .event-card:first-of-type', SiteConfig.evento.ceremonia);
    applyEventCard('#recepcion', SiteConfig.evento.recepcion);
    applyFooterConfig();
...
function initCountdownParallax() {
    ...
    const countdownItems = Array.from(document.querySelectorAll('.hero-countdown div'));
    if (!calendarBtn && countdownItems.length === 0) return;

    function updateCalendarParallax() {
        const offset = window.scrollY * 0.04;
        if (calendarBtn) calendarBtn.style.setProperty('--calendar-parallax', offset + 'px');
        countdownItems.forEach((item, index) => {
            const direction = index % 2 === 0 ? 1 : -1;
            item.style.setProperty('--countdown-parallax', (window.scrollY * 0.018 * direction) + 'px');
        });
    }
...
function initEventButtonsParallax() {
    ...
    const ceremonyCards = Array.from(document.querySelectorAll('.ceremony-card'));
    if (buttons.length === 0 && ceremonyCards.length === 0) return;
    ...
    function updateEventButtonsParallax() {
        ...
        ceremonyCards.forEach((card, index) => {
            const direction = index % 2 === 0 ? -1 : 1;
            card.style.setProperty('--ceremony-card-parallax', (window.scrollY * 0.008 * direction) + 'px');
        });
    }
...
function initRSVP() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        if (isCheckingStatus) {
            showResultPopup('Estamos validando tu estado de confirmación. Intenta de nuevo en un momento.', true);
            return;
        }
    ...
    async function processConfirmation(respuesta, confirmedCount) {
        ...
        try {
            ...
            if (error && error.code === 'RSVP_GROUP_FULL') {
                showResultPopup('Lo sentimos, el cupo para este grupo ya está lleno.', true);
                if (submitBtn && !formLocked) submitBtn.disabled = false;
                return;
            }
            ...
            if (submitBtn && !formLocked) submitBtn.disabled = false;
            showResultPopup('No pudimos guardar tu confirmación. Intenta nuevamente.', true);
        }
    }
}
